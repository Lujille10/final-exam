...
foreach (['role' => "ALTER TABLE accounts ADD COLUMN role VARCHAR(50) DEFAULT 'Staff' AFTER username",
          ...
          'full_name' => "ALTER TABLE accounts ADD COLUMN full_name VARCHAR(150) DEFAULT '' AFTER email"] as $col => $alter) {
    $c = $conn->query("SHOW COLUMNS FROM accounts LIKE '$col'");
    if ($c->num_rows === 0) $conn->query($alter);
}
...
// ── Role check helper ─────────────────────────────────────────────────────
// Refresh role from DB each request so it stays accurate
$currentRole = 'Staff';
...
$roleStmt->fetch();
$roleStmt->close();
$_SESSION['role'] = $currentRole; // keep session in sync

$isAdmin = ($currentRole === 'Administrator');

// ── ADD USER — admin only ─────────────────────────────────────────────────
$message = $errors = '';
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'add_user') {
    ...
    if (empty($fullName) || empty($un) || empty($pass)) {
        ...
        $chk->bind_param("s", $un); $chk->execute(); $chk->store_result();
        if ($chk->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                logActivity($conn, $_SESSION['user_id'], $_SESSION['username'] ?? 'Admin', 'Added', 'add', "Added new user \"$un\" with role $role");
            } else { $errors = "Failed to add user."; }
            $stmt->close();
        }
    }
}
...
if ($isAdmin && isset($_GET['delete_user']) && is_numeric($_GET['delete_user'])) {
    $delId = (int)$_GET['delete_user'];
    if ($delId === (int)$_SESSION['user_id']) {
        $errors = "You cannot delete your own account.";
    } else {
        $stmt = $conn->prepare("DELETE FROM accounts WHERE id = ?");
        $stmt->bind_param("i", $delId); $result = $stmt->execute(); $stmt->close();
        if ($result) {
            $message = "User deleted successfully.";
            logActivity($conn, $_SESSION['user_id'], $_SESSION['username'] ?? 'Admin', 'Deleted', 'delete', "Deleted user ID $delId");
        } else { $errors = "Failed to delete user."; }
    }
} elseif (!$isAdmin && isset($_GET['delete_user'])) {
    $errors = "Access denied. Only Administrators can delete users.";
}

// ── EDIT USER — admin only ────────────────────────────────────────────────
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'edit_user') {
        ...
        if (!empty($pass)) {
            if (strlen($pass) < 6) {
                ...
                if ($result) {
                    ...
                    logActivity($conn, $_SESSION['user_id'], $_SESSION['username'] ?? 'Admin', 'Edited', 'edit', "Updated user \"$un\"");
...
} elseif (!$isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'edit_user') {
    $errors = "Access denied. Only Administrators can edit users.";
}
...
<div class="page-body">
    ...
    <!-- Staff notice -->
    <?php if (!$isAdmin): ?>
        <div class="alert" style="background:rgba(0,188,212,0.1);border:1px solid rgba(0,188,212,0.25);
                                   color:rgba(255,255,255,0.6);font-size:0.82rem;margin-bottom:1rem;">
            🔒 You have <strong>view-only</strong> access. Only Administrators can add, edit, or delete users.
        </div>
    <?php endif; ?>
                        ...
                        <td>
                            ...
                            <?php if ((int)$u['id'] === (int)$_SESSION['user_id']): ?>
                                <span class="badge badge-blue"
                                      style="margin-left:0.4rem;font-size:0.6rem;">● You</span>
                            <?php endif; ?>
                        </td>
                            ...
                            <div class="action-btns">
                                ...
                                <button class="btn btn-warning btn-sm"
                                        onclick="openEditUser(
                                            ...
                                            '<?= htmlspecialchars(addslashes($u['role'] ?? 'Staff')) ?>'
                                        )">✏️</button>
                                <!-- Delete -->
                                <?php if ((int)$u['id'] !== (int)$_SESSION['user_id']): ?>
                                ...
                                <?php else: ?>
                                    <button class="btn btn-danger btn-sm" disabled
                                            ...
                                            title="Cannot delete your own account">🗑</button>
                                <?php endif; ?>
                            </div>
                        </td>
                        <?php endif; ?>
...
<div id="add-user-modal"
     style="display:none;position:fixed;inset:0;z-index:999;background:rgba(0,0,0,0.7);
    ...
    <div class="form-card" style="width:100%;max-width:500px;animation:cardReveal .4s both;">
        <div class="form-card-header" style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <button onclick="document.getElementById('add-user-modal').style.display='none'"
                    style="background:none;border:none;color:var(--text-dim);font-size:1.3rem;cursor:pointer;">✕</button>
        </div>
        <form method="POST" class="form-card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="username" class="form-control"
                           placeholder="e.g. juandc" required>
                </div>
            </div>
            <div class="form-group">
                <label>Email Address</label>
                <input type="email" name="email" class="form-control"
                       placeholder="juan@wearetheoceans.com">
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Role</label>
            ...
            <div style="display:flex;gap:.75rem;justify-content:flex-end;margin-top:.5rem;">
                <button type="button" class="btn btn-secondary"
                        onclick="document.getElementById('add-user-modal').style.display='none'">Cancel</button>
    ...
    <div class="form-card" style="width:100%;max-width:500px;animation:cardReveal .4s both;">
        ...
        <form method="POST" class="form-card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label>New Password
                        <small style="color:var(--text-muted);font-weight:400;">(leave blank to keep)</small>
                    </label>
                    <input type="password" name="password" class="form-control"
            ...
            <div style="display:flex;gap:.75rem;justify-content:flex-end;margin-top:.5rem;">
                <button type="button" class="btn btn-secondary"
                        onclick="document.getElementById('edit-user-modal').style.display='none'">Cancel</button>
...
<?php endif; // isAdmin — modals + JS only rendered for admin ?>
